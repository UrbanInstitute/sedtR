...
The `sedtR` package to allow R programmers to easily interact with Urban's [Spatial Equity Data Tool](https://apps.urban.org/features/equity-data-tool/) (SEDT) application programming interface (API). 
...
`call_sedt_api()` returns a list object contains a `sf` object that can be visualized using `create_map()`:
